...
`node "${HARNESS_ROOT}/tools/spec-lint/spec-lint.mjs" gate --uc UC-nnn` decides all three in one run. Marking a contract `fixed` is done **by the orchestrator** upon a fully checked `artifacts` report from structure-oracle returning zero inconsistencies (§3 Phase3; never from proposal review).
...
- **Launch order**:
  ...
  4. `structure-oracle` (🔴) — independent judgment in a separate context, with `review mode = artifacts`. For a shared-concept or structural change, pass the complete concept-wide scope collected in §4, including affected completed slices, rather than judging each UC in isolation. Iterate to zero inconsistencies (round limit in §4). **Its mission is the semantic half only** — the format, the transport-field agreement, the vocabulary resolution, the example integrity (spec-lint), and placement / dead rules / duplicate IDs (trace-check) are already decided by machine, so do not have it re-read contracts to confirm those
- **Marking the contract `fixed`**: only when structure-oracle returns a fully checked `artifacts` report with zero inconsistencies and the required machine checks pass, **the orchestrator** sets `x-status` in `contract.yaml` to `fixed` and advances the UC's `phase:` to `構造` (no human approval in between; never let a producer or oracle mark it `fixed` either).
...
**Measure before dispatch (orchestrator, inline).** Before dispatching a change to a rule's scope, a shared concept, or a structural decision, including during definition, collect its blast radius as one batch:

1. Search the host's docs, declared native schema, source, and tests together with `rg -l -F -e '<ID>' -e '<term>' -e '<code identifier>' <existing roots...>` (or equivalent grep). Use the decision's IDs, old/new terms, and known aliases; inspect matching lines as needed. Include shared vocabulary and completed slices, not just the current UC. Follow newly found references and symbols until no new affected addresses emerge. Generated/vendor copies are not source targets; record any relevant roots the search could not cover.
2. Keep a temporary dispatch map in the working context: affected paths, intended change, responsible producer, read inputs, write targets, dependencies, and re-verification scope. Search hits are candidates, not automatic edits: distinguish live dependents from withdrawn artifacts and historical explanations that must remain. A zero-hit search or green `trace-check` is not proof of semantic consistency; let producers and the independent oracle read the affected artifacts.
3. Before dispatching fixes, apply **Decision preflight** below when its trigger holds. Incorporate any newly discovered dependents into the map. Route the complete set using the table below and prepare one coordinated batch, grouped by producer and dependency. Pass each Task its affected paths, the decision/intent, and predecessor outputs it must wait for. Preserve human gates and status ownership. **One batch does not mean simultaneous launch**: finish upstream writes and required approvals before launching downstream readers; parallelize only independent Tasks under the concurrency conditions below.
4. On receipt, expand the same map if a producer discovers another dependent; pause any affected downstream work and relaunch it against settled inputs. Do not knowingly continue with stale inputs or send already-known findings one at a time.
5. After the batch settles, repeat the search for missed live dependents and hand the full affected scope to the existing phase's independent oracle and machine checks, once per round. Preserve legitimate historical references; do not aim for zero textual matches. Keep the map temporary, never a committed ledger.
...
| A new table cell, a new BR, or an SSOT change that touches the frozen structure | Phase1 (update the UC / REQ / BR) → re-run Phase3 (structural consistency) → **re-verify the consistency of already-completed slices too**, using the dispatch map above (`trace-check` C4 checks dead rules; it is not a complete reverse-reference search) |

### Decision preflight (shared concepts and existing structure only)

- **Trigger**: a proposed decision adds, changes, or withdraws a shared concept/rule, or alters an existing structural assumption (state, boundary, or persistence constraint). This applies during definition too. A local correction derived from unchanged SSOT does not add a preflight; use the existing review loop.
- After collecting the dispatch map and **before any writes implementing the decision**, launch `structure-oracle` read-only in a separate context with `review mode = proposal`. Pass the proposed before/after intent (inline only if not yet in a file), its grounds, and the concept-wide paths: affected UC directories, REQs including verification policies, BRs, glossary, shared vocabulary, contracts, native schema, and relevant source/test paths found by the search. State which artifacts are not yet authored or are inapplicable and why. Pause writers to these inputs for the review. In `/develop-light`, a decision meeting this trigger is routed to mainline before this launch; light's oracle policy stays unchanged.
- Ask for counterexamples to the proposed decision across the affected concept, including semantic dependents without a direct ID match. The search map is a starting set, not a boundary of truth. Keep the existing round limit; a material revision to the proposal or its scope is re-judged before dispatch.
- **On receipt**: inspect the grounds, expand the map, and resolve all blocking findings or missing evidence before dispatch. If future design details are needed, ask the responsible producer for a design proposal as a report without changing the reviewed artifacts, then re-judge the supplemented proposal; do not wait for a downstream implementation that preflight itself blocks. Report policy ambiguity to the human with the competing interpretations; the oracle neither chooses policy nor grants approval. An empty, fully checked proposal report permits the normal producer/gate sequence only: **never mark a contract `fixed` or advance a phase from proposal review**. Existing user authorization remains applicable to the same decision; AI review does not create an extra human approval gate.
- Review the resulting artifacts in Phase3 after the batch settles, over the same expanded concept-wide scope. Proposal review does not replace this review. Mechanically executable counterexamples go to the existing test-design/implementation loop where applicable; do not add semantic heuristics to `trace-check`.
...
1. **Inputs are settled** — each Task's input is already `active` / settled / `fixed` (SSOT / DB / contract) and does not wait on another Task's output in the same round. Compare read inputs against every concurrent Task's write targets, including the orchestrator's writes: **never overlap a writer and a reader of the same file**, even when their write targets are disjoint. A status label does not make a file settled while it is being revised; launch the reader only after the writer finishes and any required approval is complete.
...
| `structure-oracle` | decision preflight (§4) / P3 after producers | review mode (`proposal` / `artifacts`), concept-wide paths; proposal + grounds for preflight (on re-judgment: previous findings + changed proposal/artifacts) | 🔴 independent judgment |
...
| proposal review report | structure-oracle (`proposal`) | follow §4 Decision preflight; unresolved findings or unchecked evidence block dispatch. No status or phase transition |
| artifact inconsistency list | structure-oracle (`artifacts`) | 🤖 send all items back in one round → re-judge on the diff scope (bounded by the circuit breaker; 🙋 if the cause is a human oracle). **Only an empty, fully checked artifact report permits the orchestrator to mark the reviewed contracts `fixed`** |
