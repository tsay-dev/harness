...
>
> **Role**: While this skill is active you are the orchestrator. You do mechanical relocation yourself (the oracle is a machine); you delegate every content conversion to the producer that owns the format (`spec-author` for domain / UC / REQ / BR / contract, `test-author` for partition classes and annotations, `implementer` for `@implements`), in separate contexts. ADRs and commits you write yourself, following the develop skill's [adr.md](../develop/references/adr.md) / [commit.md](../develop/references/commit.md) (A-1 binds the ADR content too). **Language**: these instructions are in English, the output is not. Report to the human in Japanese; migrated docs stay in Japanese. State this in every Task input.
...
- **A-4 Phase gate.** Stop at the end of every phase and have the human review the deliverables before the next. One phase = one commit (or commit group, per [commit.md](../develop/references/commit.md)); never mix changes across phases.
...
- **Never silently drop information.** Anything with no home in the new layout is routed per the develop skill's [playbook.md](../develop/references/playbook.md) (section *Routing information*: ADR, issue, commit message, attic) and named in both the plan and the report.
...
## Phase 2: vocabulary, actors, goals (`spec-author`, target `domain`)

Delegate to **`spec-author`** ([spec-author.agent.md](../../agents/develop/spec-author.agent.md), target `domain`) with the inventory, the glossary material, and the existing material as paths, and the instruction **"extract, do not invent (A-1)"**:
...
## Phase 3: use cases and the state × event table (`spec-author`, target `usecase`)
...
## Phase 4: requirements and business rules (`spec-author`, target `requirements`)

1. Delegate to **`spec-author`** (target `requirements`) per active UC (concurrently; A-1): each reserved cell becomes one `REQ-nnn.md` (EARS, one sentence) directly under the UC directory. **The REQ IDs are the ones the UC's table reserved in Phase 3** — pass them as input; the spec-author does not number REQs itself. BR numbers come from `--next br`, which is atomic, so concurrent definers may extract BRs at the same time. On the old layout, `spec.md`'s `業務ルール` bullets are the primary source (one bullet ≈ one REQ or one BR); its `受け入れ条件` (GWT) are evidence for the cell mapping, not requirements. Where an existing test observes the behavior, that observation is the sentence's grounds. `## 検証方針` stays the scaffold until Phase 6.
...
3. `git rm` the emptied old directories (`docs/specs/`), re-point every remaining reference to old paths (the project's `CLAUDE.md`, README, CI config, hooks), drop `--ignore-legacy-layout` from CI, run `spec-lint validate` (no flag; `.spec-baseline.json` empty or every entry a repayment issue) and `trace-check` one last time, and commit per [commit.md](../develop/references/commit.md) (intent: docs layout migration, with a summary of the mapping).
