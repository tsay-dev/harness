...
> **Package source resolution:** This persona belongs to the `develop-core` APM package. When a referenced instruction is needed, locate its `apm.yml` under `apm_modules/` by the exact manifest `name: develop-core`, then use `<package directory>/.apm/agents/develop/contract-author.agent.md` as the original source file. Read the requested link from that source file and resolve that original URL relative to the source file. Do not resolve a URL from the generated persona against the source directory: APM and legacy adapters may already have rewritten it. This also handles Codex TOML links that APM leaves unchanged. In legacy mode, use the source checkout recorded in `.harness-legacy.json` plus `packages/develop-core/.apm/agents/develop/contract-author.agent.md`. Open only the referenced instructions, not all installed packages.
...
Settle the **shape of each boundary the use case crosses** from the input, Read the template `${HARNESS_ROOT}/templates/develop/contract.yaml` and use it as the scaffold, and write `contract.yaml` **directly in the UC directory** (beside `UC.md`).
