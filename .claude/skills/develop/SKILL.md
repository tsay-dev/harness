...
- **Launch order**:
  ...
  5. Once FE and BE implementations are both in, advance `phase:` to `検証` and launch **`slice-reviewer`** (🔴; **top** tier — §5 / ADR-0024). Iterate to zero defects (round limit in §4). **`slice-attacker` / `system-attacker` are not launched in this phase.**
...
**Model per agent — three tiers, and the tier is the agent's own `model:`.** The assignment lives in `.claude/agents/develop/<name>.md` frontmatter (`opus` = **top**, `sonnet` = **mid**, `haiku` = **light**); this section only says what each tier means in the runtime you are in (ADR-0024). Never restate a per-agent assignment here or in another skill — Read the frontmatter.

| Tier | Who | Claude Code | Cursor (at Task launch) | Grok Build | Codex |
| --- | --- | --- | --- | --- | --- |
| **top** | `domain-definer` / `usecase-definer` / `db-designer` / `structure-oracle` / `slice-reviewer` / `slice-attacker` / `system-attacker` | `opus` | the **opus** family | the flagship **grok** reasoning family (the non-`fast` one) | `tiers.top` in `models.json` |
| **mid** | `requirement-definer` / `contract-author` / `test-designer` / `adr-writer` / the 3 implementers / `skeleton-runner` | `sonnet` | the **sonnet** family | the **grok … fast** reasoning family | `tiers.mid` |
| **light** | `committer` | `haiku` | the **haiku** family | the smallest **grok** family offered | `tiers.light` |

- **Never hardcode a versioned catalog id** (a Cursor kebab-case slug, `grok-4.6`, and the like) into the harness or into a Task prompt. A family name used as a launch-time selection criterion (`opus` / `sonnet` / `haiku` / `fable`) is not that pin. The single exception is Codex, whose catalog IDs live in `.claude/tools/codex-sync/models.json` and nowhere else (ADR-0022 / ADR-0025).
- **Master / orchestrator** (this session, not a Task): the strongest family the runtime offers — in Cursor, **fable**. Never pass it into a Task; a child that inherits it drags the whole fleet up to it.
- **In Cursor, choose the model per Task at each launch.** A projected agent's `model:` is normalized to `inherit`, so `inherit` is forbidden: pick the family the tier names from the candidates the Task's `model` argument offers at that launch, taking the latest when several versions of that family are offered. When the human names a model, follow it.
- **When the tier's family is not among the candidates, do not silently fall back.** Tell the human which models were offered and stop, unless the human names a substitute. A weaker `slice-reviewer` is the sharpest case: a false defect list becomes a wrong implementation, and a false empty list is a false green of the definition of done.

**In Grok Build, launch specialists with the subagent spawn tool (`task` / `spawn_subagent`), not the Cursor/Claude Task tool.** After `./init.sh grok`, each persona is flattened to `.grok/agents/<name>.md`; `subagent_type` is that frontmatter `name:` (for example `domain-definer`, `translate-manga-ko-ja-maker`). The projection drops `model:`, so the child inherits the parent — Grok's spawn API has no per-launch model argument, and the tier table above is therefore a statement about which model the human should be running this session on. When a **top** agent is due and the session is below it, say so instead of proceeding silently. The Claude Code tool names in agent frontmatter are ignored; the read-only discipline is in each agent body.

**In Codex, launch specialists with `spawn_agent`, not the Cursor/Claude Task tool.** After `./init.sh codex`, each persona is flattened to `.codex/agents/<name>.toml`; the TOML `name` is the role (`domain-definer`, `translate-manga-ko-ja-maker`, …). Catalog IDs are not in this skill — Read `.claude/tools/codex-sync/models.json` (ADR-0022 / ADR-0025). **The parent session's model is whatever the human started you on** (do not ask them to switch to the top pin — ADR-0023). Each TOML already carries the pin `models.json` gives that agent's tier, read from its `model:` frontmatter (`opus` → `tiers.top`, `sonnet` → `tiers.mid`, `haiku` → `tiers.light`; anything else → `default_tier`). How you pass the role depends on the schema the runtime exposes at that launch:
...
- The TOML pin may be ignored on some Codex surfaces; then the child inherits the parent, and the tier is only a statement about which model this session needs to be on.
...
| `slice-reviewer` | P4, after implementation | the slice, the UC directory, BE tests, change scope (on re-verification: previous defects + change scope) | 🔴 independent judgment (**top** tier) |
