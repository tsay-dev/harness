...
# ADR-0027 共有リポジトリを harness に改名し、ホストの submodule パスは新設を .harness・既存は .claude-harness のまま更新する

<!-- File name: docs/adr/ADR-0027-rename-to-harness.md (the prefix must equal id). One ADR, one decision. Never rewrite an accepted ADR — add a new one with supersedes: (R-802). Write the content in Japanese. -->
